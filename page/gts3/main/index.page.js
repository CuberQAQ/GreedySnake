...
Page({
  build() {
    ...
    // 方块位置的计算
    const BACKGROUND_LENGTH = px(420) //背景矩形的边长
    ...
    let tempOffsetLengthY = (px(480) - BACKGROUND_LENGTH) / 2 + BLOCK_DISTANCE + px(50)
    ...
    let timerInit = timer.createTimer(
      ...
      option => {
        ...
        var rectBackground = hmUI.createWidget(hmUI.widget.FILL_RECT, {
          x: (px(480) - BACKGROUND_LENGTH) / 2,
          y: (px(480) - BACKGROUND_LENGTH) / 2 + px(50),
          radius: px(5),
          w: BACKGROUND_LENGTH,
          h: BACKGROUND_LENGTH,
    ...
    function changeSize(blockX, blockY) {
      if(data.json.anim.size) {
        ...
        newFx.func = function (d) {
          ...
          let y = (px(480) - BACKGROUND_LENGTH) / 2 + BLOCK_DISTANCE + px(50)
